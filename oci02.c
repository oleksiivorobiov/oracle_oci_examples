...
/*
   ...
   DESCRIPTION
     ...
     Tests single row fetch of a query with a bind by position
   MODIFIED   (MM/DD/YY)
    kmohan     03/28/06  - change hda to size_t
    aliu       01/05/06  - add order by in the select to fix intermittent 
                           diffs 
 ...
 *
 * Based on:    oci01.c
 * Changes:     added obndrn call
 *              added where clause to query with :1
 *
