...
/*
   ...
   DESCRIPTION
     ...
     Tests array fetch using PL/SQL procedures via PL/SQL arrays
   MODIFIED   (MM/DD/YY)
...
text *sqlstmt = (text *)"\
                 begin \
                   oci10pkg.oci10proc(:cname, :collen, :colid, :tabname,\
                                      :batch_size, :num_ret, :done_fetch);\
                 end;";
...
void setup()
{
  ...
  if (obndra(&cda, (text *)":batch_size", -1,
             (ub1 *)&numrows, (sword)sizeof(numrows),
  ...
  if (obndra(&cda, (text *)":num_ret", -1,
             (ub1 *)&num_ret, (sword)sizeof(num_ret),
             SQLT_INT, -1, (sb2 *) 0, (ub2 *)0, (ub2 *)0,
             (ub4)0, (ub4 *)0, (text *)0, -1, -1))                   /* bind */
  {
    err_report(&cda);
  ...
  if (obndra(&cda, (text *)":done_fetch", -1,
             (ub1 *)&done_fetch, (sword)sizeof(done_fetch),
             SQLT_INT, -1, (sb2 *) 0, (ub2 *)0, (ub2 *)0,
             (ub4)0, (ub4 *)0, (text *)0, -1, -1))                   /* bind */
  {
    err_report(&cda);
...
#ifdef BUG207799
                            /* bug 207799 - need to reset the curlen to zero */
    colsptr = cols;
    for (colindex = 0; colindex < NUMCOLS; colindex++,colsptr++)
       colsptr->c_curlen = 0;
#endif /* BUG 207799 */
...
void dump_data()
{
  ...
  uword index;
  ub4 value;
  for (index = 0; index < num_ret; index++)
  {
  ...
  row_count += num_ret;                        /* we only need the increment */
