#ifdef RCSID
static char *RCSid =
   "$Header: cdemoup2.c 20-oct-99.14:15:13 svedala Exp $ ";
#endif /* RCSID */
...
/*
   ...
   NAME
     cdemoup2.c - User callback library routine 2.

   DESCRIPTION
     The library cdemoup2 for user callback is created using this
     file. The command for compiling and creating the library is:

       make -f ociucb.mk user_callback SHARED_LIBNAME=cdemoup2.so.1.0 \
                                                      OBJS=cdemoup2.o
...
sword cdemoup2EnvCallback(OCIEnv *env, ub4 mode, size_t xtramemsz, dvoid
                         *usrmemp, OCIUcb *ucbDesc);
...
/*
   NAME:
       cdemoup2Init - OCI Shared Library CallBack Main function
...
sword cdemoup2Init(metaCtx, libCtx, argfmt, argc, argv, envCallback)
...
/* ------------------------------------------------------------- */
sword cdemoup2EnvCallback(env, mode, xtramemsz, usrmemp, ucbDesc)
OCIEnv  *env;
ub4     mode;
size_t  xtramemsz;
dvoid   *usrmemp;
...
/* function is registered dynamically from function cdemoup2EnvCallback */
/* -------------------------------------------------------------------- */
                                ...
                                sb4 *errnop, va_list arglist)
{
  ...
  printf("Library%d: In dynamic entry callback function for OCIStmtPrepare:\n",
                                                                           id);
                               ...
                               sb4 *errnop, va_list arglist)
{
  ...
  printf("Library%d: In dynamic exit callback function for OCIStmtPrepare:\n",
                                                                          id);
  printf("Library%d: sql_stmt = [%s]\n", id, sqlstmt);
  printf("Library%d: Context string = [%s]\n", id, loc_ctx->str1);
