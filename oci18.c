...
/*
   ...
   DESCRIPTION
     ...
     Tests array insert via PL/SQL function
   MODIFIED   (MM/DD/YY)
...
text *sqlstmt = (text *)"\
                        begin \
                            :retval := oci18pkg.oci18func(:col1, :numins);\
                        end;";

#define NUMCOLS 1               /* number of columns returned by above query */
#define MAX_ROWS_PER_INSERT 5                      /* 5 rows max per insert */

uword numinsert;             /* number of rows to insert on the current pass */
uword retval;                                  /* return value from function */
